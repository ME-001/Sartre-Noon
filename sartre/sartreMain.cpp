...
int main(int argc, char *argv[])
{
    ...
    for (unsigned long iEvent = 0; iEvent < maxEvents; iEvent++) {
        ...
        *gamma   = event->particles[3].p;
        ...
        f(settings->UPC() and settings->A()==settings->UPCA()){ // this is to ensure that this part of the code run only for UPC
       //loading the Neutron generator only once 
        ...
        {	
        	...
        	gg[0]->neutronRecord(nNumbers[0], nNumbers[1], 0,E,Px,Py,Pz);
        	for(Int_t x =0; x< nNumbers[0];x++){
        	Particle& particle = event->particles[7+x];
        	TLorentzVector* neutron1 = new TLorentzVector(Px[x], Py[x], Pz[x], E[x]);
        	particle.p = *neutron1;
        	particle.pdgId = 2112;
        	particle.parents = {0};
        	particle.status =1;
        ...
        {	
        	vector<Double_t> E,Px,Py,Pz; 
        	gg[0]->neutronRecord(nNumbers[0], nNumbers[1], 1,E,Px,Py,Pz);
        	for(Int_t x = 0;x<nNumbers[1];x++){
        	Particle& particle = event->particles[7+nNumbers[0]+x];
        	TLorentzVector* neutron2= new TLorentzVector(Px[x], Py[x], Pz[x], E[x]);
        	particle.p = *neutron2;
        	particle.pdgId = 2112;
        	particle.parents = {1};
        	particle.status =1;
