...
NeutronGenerator::NeutronGenerator(int beamgamma)
  : fRunMode(kInterface)
  ...
  , nucleus_Z(82)
  , nucleus_A(208)
  , beamGamma(beamgamma) //Change this value according to your need
  ...
  , kStoreQA(kTRUE) // modifying it to be true in the acessing file
  , kStoreGen(kTRUE) // same as above 
{
 // Default constructor
...
/*void NeutronGenerator::SetBeamGamma(double bm)
{beamGamma =bm;
gammaTarget =2.0*beamGamma*beamGamma-1.0;
Initialize();
ReadENDF(kTRUE);
...
//______________________________________________________________________________
void NeutronGenerator::CreateNeutrons(const Int_t nNeutronsBeam1, const Int_t nNeutronsBeam2) // need modification
{
...
//==========================================================================================================================
/**
 * Our methods runSartreNoon and createSartreNeutrons
 * runSartreNoon is modified version of generateEvent
 * createSartreNoon is modified version of CreateNeutrons  */ 
//=======================================================================================================================
std::vector<int> NeutronGenerator::runSartreNoon(const Double_t photonK) 
{
...
//
//  Recently created function to get the LorentzVector of neutrons for each side separetely
//
void NeutronGenerator::neutronRecord(const Int_t nBeam1, const Int_t nBeam2, const Int_t side, std::vector<TLorentzVector> &V)
{
	//hNeutronMultiplicity->Fill(nBeam1,nBeam2);
  	Double_t energyKin = 0,mom = 0,phi = 0, theta = 0;
  	...
  	if(nNeutrons[side] == 0){}
    	...
    	for(Int_t i = 0; i<nNeutrons[side]; i++){
      	...
      	vec.SetPxPyPzE(vec.Px()*0.001,vec.Py()*0.001,vec.Pz()*0.001,vec.Energy()*0.001);  
      	//vec.SetPxPyPzE(vec.Px(),vec.Py(),vec.Pz(),vec.Energy());  
...
void NeutronGenerator::BuildPhotonFluxModulationTables()
{
  ...
  std::cout<<"Building photon flux modulation tables"<<std::endl;  
...
void NeutronGenerator::BuildNucleusBreakupProbabilityTable()
{
  ...
  std::cout<<"Building nucleus breakup probability tables"<<std::endl;  
...
void NeutronGenerator::BuildHadronicInteractionProbabilityTable()
{
  ...
  std::cout<<"Building hadronic interaction probability table for "<<hadrModel.Data()<<" model"<<std::endl;
  for(Double_t impactPar_value = nucleus_R; impactPar_value<=25.0; impactPar_value *= impactPar_delta){
    ...
    if(Int_t(iStep/1.91)%10 == 0){
      if(iStep != 1){ printf("\033[1A"); printf("\033[K");}   
       std::cout<<Int_t(iStep/1.91)<<"%"<<std::endl;
       }
    }
...
void NeutronGenerator::ReadENDF(Bool_t saveToFile)
{
  ...
  std::ifstream infile("ENDF_Pb208.txt");
