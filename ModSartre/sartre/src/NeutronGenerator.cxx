...
//______________________________________________________________________________
void NeutronGenerator::CreateNeutrons(const Int_t nNeutronsBeam1, const Int_t nNeutronsBeam2, Double_t beamGamma) // need modification
{
...
void NeutronGenerator::createSartreNeutrons(const Int_t nBeam, Int_t index, Event &event, Int_t side, TClonesArray &neutron, Double_t beamGamma)
{
...
//______________________________________________________________________________
//
// Most Important Part of NOON DOn't edit or change
//______________________________________________________________________________
void NeutronGenerator::BuildPhotonFluxModulationTables(Double_t beamGamma)
{
...
//______________________________________________________________________________
Double_t *NeutronGenerator::PhotonFlux(const Double_t energyPhoton, Double_t beamGamma)
{
...
//______________________________________________________________________________
Double_t NeutronGenerator::PhotonDensity(const Double_t distance, const Double_t energyPhoton, Double_t beamGamma)
{
...
//______________________________________________________________________________
Double_t NeutronGenerator::PhotonDensity2(const Double_t distance, const Double_t energyPhoton Double_t gammaTarget)
{
...
//______________________________________________________________________________
void NeutronGenerator::BuildNucleusBreakupProbabilityTable(Double_t beamGamma)
{
  ...
  for(Double_t impactPar_value = impactPar_min; impactPar_value<=impactPar_max; impactPar_value *= impactPar_delta){
    Double_t* prob_Breakup = NucleusBreakupProbability(impactPar_value,2*beamGamma*beamGamma-1, beamGamma);
...
//______________________________________________________________________________
Double_t *NeutronGenerator::NucleusBreakupProbability(const Double_t impactPar, Double_t gammaTarget, Double_t beamGamma)
{
  ...
  gk1m = PhotonDensity2(impactPar,energyGamma_Xn[0],gammaTarget);
  for(Int_t k = 1; energyGamma_Xn[k] < maxPhotonEnergy; k++){
    gk1 = PhotonDensity2(impactPar,energyGamma_Xn[k],gammaTarget);
  ...
  for(Int_t i=0; i<10; i++){
    ...
    gk1m = PhotonDensity2(impactPar,gSection_Nn[i].GetX()[0],gammaTarget);
    for(Int_t k = 1; gSection_Nn[i].GetX()[k] < maxPhotonEnergy; k++){
      gk1 = PhotonDensity2(impactPar,gSection_Nn[i].GetX()[k],gammaTarget);
...
//______________________________________________________________________________
//
void NeutronGenerator::Initialize(Double_t beamGamma)
{
  Double_t gamamTarget = 2*beamGamma*beamGamma-1;
 ...
 Double_t energyGamma_1n[76] = {7.5, 7.57, 7.64, 7.71, 7.79, 7.88, 7.95, 8.05, 8.12, 8.19, 8.26, 8.33, 8.47, 8.56, 8.65, 8.74, 8.87, 8.98, 9.12, 9.23, 9.35, 9.49, 9.6, 9.74, 9.84, 9.96, 10.08, 10.22, 10.37, 10.49, 10.61, 10.73, 10.85, 11.0, 11.13, 11.24, 11.38, 11.51, 11.64, 11.76, 11.89, 12.0, 12.13, 12.26, 12.53, 12.68, 12.95, 13.12, 13.5, 13.79, 14.05, 14.28, 14.58, 14.8, 15.08, 15.37, 15.66, 15.95, 16.21, 16.53, 16.79, 17.05, 17.33, 17.6, 17.86, 18.13, 18.4, 18.66, 18.9, 19.2, 19.46, 19.74, 20.03, 20.28, 20.55, 20.85}; 
  ...
  if(fRunMode != kFlatMultiplicity && fRunMode != k1n1n){
    BuildHadronicInteractionProbabilityTable(beamGamma);
    BuildNucleusBreakupProbabilityTable(beamGamma);
    BuildPhotonFluxModulationTables(beamGamma);
    }
  InitQAhistograms();
}

//______________________________________________________________________________
void NeutronGenerator::BuildHadronicInteractionProbabilityTable(Double_t beamGamma)
{
  Int_t iStep = 0;
  ...
  for(Double_t impactPar_value = nucleus_R; impactPar_value<=25.0; impactPar_value *= impactPar_delta){
    probValue = TMath::Exp(-1.0*HadronicInteractionProbability(impactPar_value, beamGamma));
...
//______________________________________________________________________________
Double_t NeutronGenerator::HadronicInteractionProbability(const Double_t impactPar, Double_t beamGamma)
{
