...
void randomlyReverseBeams(Event* );  // for UPC mode // might change this 
...
int main(int argc, char *argv[])
{
    ...
    // tree.Branch("Neutrons1 ","TLorentzVector",&neutrons,32000,0);
    // tree.Branch("Neutrons2","TLorentzVec");
    tree.Branch("neutronsSide1", &neutrons1);
    tree.Branch("neutronsSide22", &neutrons2);
    ...
    for (unsigned long iEvent = 0; iEvent < maxEvents; iEvent++) {
        ...
        // accessing vector meson rapidity

        Double_t y = vm->Rapidity();

        //Calculating photont k
        ...
        {
            gen->createSartreNeutrons(n1, 7+i, *event, 0, neutrons1,eIn->Gamma());   
        }

        for(Int_t j = 0 ; j <n2 ; j++)
        {
            gen->createSartreNeutrons(n2, 7+n1+j, *event, 1, neutrons2,eIn->Gamma());  
        }
